...
if 'fit' in args.step or args.step == "all":
  ...
  print("Running nominal fit FOR POSTFITPLOTS")

  os.system('combineTool.py -m 125 -M MultiDimFit --redefineSignalPOIs \"%(pois_str)s\" --saveFitResult --X-rtd MINIMIZER_analytic --expectSignal 0 --cminDefaultMinimizerStrategy 0 --cminDefaultMinimizerTolerance 0.1 --algo singles --cl=0.68 --there -d outputs/%(output_dir)s/cmb/ws.root -n ".ztt.bestfit.singles.robustHesse" --freezeNuisanceGroups TES %(tes_ranges_str)s --setParameters %(tes_nom_str)s --robustHesse=1' % vars())
  ...
  # DANNY POSTFIT:
  # os.system('combineTool.py -m 125 -M MultiDimFit --redefineSignalPOIs "${pois}" --saveFitResult --X-rtd MINIMIZER_analytic --expectSignal 0 --cminDefaultMinimizerStrategy 0 --cminDefaultMinimizerTolerance 0.1 --algo none --there -n ".ztt.bestfit.singles.robustHesse" -d outputs/fit_Run3_2022/cmb/ws.root --robustHesse=1')
